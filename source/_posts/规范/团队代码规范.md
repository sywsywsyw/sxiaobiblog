...
看图片类型，内容型的用img；装饰型的用background-image

项目开发 什么图片应该压缩？ 什么图片不应该压缩？

设计师给的效果图  尽量不要压缩 看情况 如果特别大 例如banner示例图时建议压缩

用户上传的资料 超过300K以上的尽量压缩 然后后端 必须做分割操作  切成不同比例大小的图片 （可使用第三方如：七牛....）
